...
The Advanced mode computes rather than recalls, so its numbers have to be
traceable. Nothing in it is generated by a language model, and nothing is a
template with an invented number filling a gap.

| What | Where it comes from |
| --- | --- |
| Atomic weights, in every molar mass, percent composition and mole conversion | IUPAC 2021 standard atomic weights, through `elements.json` — the same values the rest of the app uses |
| The Avogadro constant, 6.02214076 × 10²³ mol⁻¹ | Exact by definition since the 2019 SI redefinition of the mole. Not a rounded 6.02 |
| Electron configurations | `elements.json`, validated as described above |
| Electronegativity comparisons | The Pauling values in `elements.json`; a pair is only asked about when both have one and they differ by at least 0.3 |
| Valence electron counts | Derived from the group number, and **only for the main group**. The d block is excluded because the count there is genuinely ambiguous, and the app does not ask questions whose answer it would have to choose |
| Common ion charges | Derived from the group: 1 → +1, 2 → +2, 13 → +3, 15 → −3, 16 → −2, 17 → −1. Group 14 is excluded because it reliably takes neither |
| Compound formulas, names and compositions | `compounds.json`, sourced as described in `COMPOUND_SOURCES.md` |

### The one curated table

`AdvancedQuestionBuilder.curatedOxidationStates` is the exception: a short list
of common oxidation states for the metals whose charges do not follow from a
group — Fe, Cu, Zn, Ag, Cr, Mn, Co, Ni, Sn, Pb, Hg, Au, Pt, Cd, Ti.

It is deliberately short. Every entry is a state that general-chemistry courses
agree on and that appears in ordinary compounds: iron(II) and iron(III),
copper(I) and copper(II), chromium(II), (III) and (VI), manganese(II), (IV) and
(VII). Anything whose "common" oxidation state depends on which course you are
taking is not in the table, because a quiz that asks it would be asking the
learner to guess which textbook the app read.

To change it: edit the table, and add the assertion to
`PeriodicProTests/AdvancedChemistryTests.swift` that fixes what you changed.

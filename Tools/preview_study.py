...
# Both appearances, read from the same tokens the app uses.
LIGHT = dict(
    CANVAS=(249, 250, 252), SURFACE=(255, 255, 255), HAIRLINE=(229, 233, 239),
    PRIMARY=(17, 22, 33), SECONDARY=(99, 109, 126), TERTIARY=(134, 143, 159),
    ACCENT=(32, 102, 229), WARNING=(216, 102, 50), POSITIVE=(22, 163, 104),
    PRO_BADGE=(224, 233, 250),
    TILES=[
...
DARK = dict(
    CANVAS=(14, 15, 19), SURFACE=(27, 29, 35), HAIRLINE=(58, 61, 71),
    PRIMARY=(242, 244, 249), SECONDARY=(156, 164, 179), TERTIARY=(122, 130, 146),
    ACCENT=(30, 130, 255), WARNING=(247, 146, 92), POSITIVE=(61, 201, 138),
    PRO_BADGE=(26, 44, 72),
    # Dark-mode family fills are low-luminance versions of the same hues, and
    # their ink is white at 94% for every family.
    TILES=[
...
def card(draw, x, y, width, height, radius=CARD_RADIUS, fill=None, stroke=None):
    draw.rounded_rectangle(
        [px(x), px(y), px(x + width), px(y + height)],
        radius=px(radius),
        fill=fill or T["SURFACE"],
        outline=stroke or T["HAIRLINE"],
        width=max(1, SCALE // 2),
    )
...
def render(name: str, width: int, height: int) -> tuple[Image.Image, float]:
    ...
    image = Image.new("RGB", (px(width), px(height)), T["CANVAS"])
    draw = ImageDraw.Draw(image)
    content = width - MARGIN * 2

    # --- chrome -----------------------------------------------------------
    draw.rectangle([0, 0, px(width), px(STATUS_BAR)], fill=T["CANVAS"])
    draw.text((px(MARGIN), px(STATUS_BAR - 24)), "9:41", font=font(BOLD, 13), fill=T["PRIMARY"])
    centered(draw, "Study", font(BOLD, 15), T["PRIMARY"], width / 2, STATUS_BAR + 12)
    y = STATUS_BAR + NAV_BAR + 8

    # --- greeting ---------------------------------------------------------
    title = font(BOLD, 33)
    draw.text((px(MARGIN), px(y)), "Good morning!", font=title, fill=T["PRIMARY"])
    y += 39
    draw.text((px(MARGIN), px(y)), "Start exploring.", font=title, fill=T["PRIMARY"])
    y += 41
    draw.text((px(MARGIN), px(y)), "Small steps. Big knowledge.",
              font=font(REGULAR, 15), fill=T["SECONDARY"])
    y += 20 + SECTION
    ...
    for index, (glyph, glyph_color, value, caption) in enumerate([
        ("flame", T["WARNING"], "0", "Day streak"),
        ("ring", T["POSITIVE"], "0%", "Elements mastered"),
    ]):
        x = MARGIN + index * (half + gap)
        card(draw, x, y, half, status_height)
        if glyph == "flame":
            draw.ellipse([px(x + 16), px(y + 16), px(x + 34), px(y + 34)], fill=T["WARNING"])
        else:
            draw.ellipse([px(x + 16), px(y + 16), px(x + 44), px(y + 44)],
                         outline=T["HAIRLINE"], width=max(1, px(4.5)))
        draw.text((px(x + half - 20), px(y + 18)), "›", font=font(BOLD, 13), fill=T["TERTIARY"])
        draw.text((px(x + 16), px(y + 46)), value, font=font(BOLD, 22), fill=T["PRIMARY"])
        draw.text((px(x + 16), px(y + 74)), caption, font=font(REGULAR, 12), fill=T["SECONDARY"])
    ...
    draw.rounded_rectangle(
        ...
        radius=px(CARD_RADIUS), fill=T["ACCENT"],
    )
    draw.rounded_rectangle(
    ...
    # --- practice -------------------------------------------------------
    draw.text((px(MARGIN), px(y)), "Practice modes", font=font(BOLD, 19), fill=T["PRIMARY"])
    ...
    # --- recent searches --------------------------------------------------
    draw.text((px(MARGIN), px(y)), "Recent searches", font=font(BOLD, 19), fill=T["PRIMARY"])
    clear_font = font(REGULAR, 13)
    draw.text((px(MARGIN + content) - draw.textlength("Clear", font=clear_font), px(y + 5)),
              "Clear", font=clear_font, fill=T["ACCENT"])
    y += 26
    ...
    for index, term in enumerate(rows):
        ...
        if index:
            draw.line([px(MARGIN + 28), px(row_y), px(MARGIN + content), px(row_y)],
                      fill=T["HAIRLINE"], width=max(1, SCALE // 2))
        draw.ellipse([px(MARGIN + 17), px(row_y + 16), px(MARGIN + 29), px(row_y + 28)],
                     outline=T["SECONDARY"], width=max(1, SCALE // 2))
        draw.text((px(MARGIN + 40), px(row_y + 14)), term, font=font(REGULAR, 16), fill=T["PRIMARY"])
        draw.ellipse([px(MARGIN + content - 30), px(row_y + 16),
                      px(MARGIN + content - 18), px(row_y + 28)],
                     outline=T["TERTIARY"], width=max(1, SCALE // 2))
    y += row_height * len(rows) + SECTION

    # --- favorites --------------------------------------------------------
    draw.text((px(MARGIN), px(y)), "Favorites", font=font(BOLD, 19), fill=T["PRIMARY"])
...
def main() -> int:
    ...
    failures = []
    sheets = []
    print(f"{'appearance':<8}{'device':<24}{'width':>7}{'tiles end':>11}{'fold':>7}  above fold")
    for appearance, theme in (("light", LIGHT), ("dark", DARK)):
        T = theme
        images = []
        for name, width, height in DEVICES:
            ...
            print(f"{appearance:<8}{name:<24}{width:>7}{tiles_bottom:>11.0f}"
                  f"{fold:>7}  {'yes' if ok else 'NO'}")
            if not ok:
                failures.append(
                    f"{appearance} {name}: the practice tiles end at {tiles_bottom:.0f}pt "
                    f"but the content area ends at {fold}pt, so they are below the fold"
                )
        sheets.append((appearance, images))

    gap = 24 * SCALE
    for appearance, images in sheets:
        total = sum(i.width for i in images) + gap * (len(images) - 1)
        background = (232, 235, 240) if appearance == "light" else (8, 9, 12)
        sheet = Image.new("RGB", (total, max(i.height for i in images)), background)
        x = 0
        for image in images:
            sheet.paste(image, (x, 0))
            x += image.width + gap
        path = out if appearance == "light" else out.replace(".png", "-dark.png")
        sheet.save(path)
